...
var_df <- load_variants(variant_dir, var_sample_names) %>%
            calculate_vafs() %>%
...
vaf_sm <- data.table(var_df)[, list(VAF_mean = mean(VAF), nvars_ref = sum(is_ref_snp), nvars_nonref = sum(!is_ref_snp)), by=sample] %>%
            ...
            reshape2::melt(., id.vars = c("sample", "replicate", "cells", "VAF_mean", "VAF_mix", "efficiency", "coverage"),
...
p1 <- ggplot(vaf_sm, aes(sample, nvars_count, colour = nvars_type, shape = nvars_type)) +
    ...
    ggtitle('Reference and non-reference variants called')
p2 <- ggplot(vaf_sm[vaf_sm$nvars_type == 'nvars_ref',], aes(sample, nvars_count / N_TOTAL_VARS)) +
    geom_point(size = 3) +
    theme_minimal() +
    theme(legend.position = 'bottom') +
    ylim(0, 1) +
    ggtitle('Fraction of reference variants called')
