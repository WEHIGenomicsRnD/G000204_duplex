...
```{r exp_efficiency, fig.width=12, fig.height=5}
eff <- vaf_sm %>% rename(., c('coverage' = 'dup_coverage'))
...
eff$est_dup_coverage <- eff$est_raw_coverage * eff$est_efficiency_wdo
...
```{r exp_raw_coverage, fig.width=12, fig.height=4}
p3 <- ggplot(eff, aes(raw_coverage, est_raw_coverage, colour=factor(VAF_mix), shape=factor(VAF_mix))) +
    geom_point() +
    theme_minimal() +
    geom_abline(slope = 1) +
    xlab('Raw coverage') +
...
p4 <- ggplot(eff, aes(raw_coverage, est_raw_coverage, colour=factor(VAF_mix), shape=factor(VAF_mix))) +
    geom_point() +
    theme_minimal() +
    geom_abline(slope = 1) +
    xlab('Raw coverage') +
...
### Expected duplex coverage and variants (re-estimated)

```{r exp_dup_coverage, fig.width=19, fig.height=4}
...
p6 <- ggplot(eff, aes(dup_coverage, est_dup_coverage, colour=factor(VAF_mix), shape=factor(VAF_mix))) +
    geom_point() +
    theme_minimal() +
    geom_abline(slope = 1) +
    ylab('Estimated duplex coverage') +
    xlab('Duplex coverage') +
    ggtitle('Estimated vs. actual duplex coverage (zoom)') +
    scale_colour_brewer(palette = 'Dark2') +
    xlim(0, 150) + ylim(0, 150)

p7 <- ggplot(eff, aes(nvars, expected_variants, colour=factor(VAF_mix), shape=factor(VAF_mix))) +
    geom_point() +
    theme_minimal() +
    geom_abline(slope = 1) +
    ylab('Estimated variants') +
    xlab('Variants called') +
    ggtitle('Estimated vs. actual called variants') +
    scale_colour_brewer(palette = 'Dark2')
