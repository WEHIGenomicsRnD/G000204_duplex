...
```{r estimate_raw_coverage, fig.width=5.5, fig.height=4.5}
...
```{r estimate_duplex_coverage, fig.width=5.5, fig.height=4.5}
...
We can see above that we over-estimate the duplex coverage, even though we *under*estimated the raw coverage. This indicates that the efficiency calculation is too optimistic in real data (which makes sense, it's a very simple model).

Case in point:

```{r estimate_efficiency, fig.width=5.5, fig.height=4.5}
...
```{r estimate_coverage_with_dropout, fig.width=5.5, fig.height=4.5}
...
ggplot(eff_nano, aes(dup_coverage, est_dup_coverage_wdo, shape=nuclease)) +
    ...
    xlim(0, 600) + ylim(0, 600)
```
There's still an over-estimate here, likely due to the over-estimation of raw coverage, however, the relationship is pretty linear, which means we can adjust for this.
